...
const OrderModal: React.FC = () => {
    ...
    const { mutate: orderWithdrawMutate, isPending } =
        useOrderWithdrawMutation();
        ...
        <ModalBase slug={modalWithdrawal}>
            <div className="bg-white p-4 flex flex-col gap-2 rounded-md w-full max-w-[500px]">
                <div className="flex items-start justify-between gap-2 mb-2">
                    <h2 className="text-xl font-medium">Заказать выплату</h2>
                    <button
                        className="ml-auto"
                        onClick={() => hideModal({ slug: modalWithdrawal })}
                    >
                        <X />
                    </button>
                </div>
                ...
                <Button
                    ...
                    disabled={card.length !== 19 || isPending || !amount}
                    size="mid"
                    theme="fill"
                >
                    Заказать
